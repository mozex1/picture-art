...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="phone"]');
...
var filter = function filter() {
  ...
  function setEventBtn(btn) {
    var mark = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    btn.addEventListener('click', function () {
      typeFilter(mark);
    });
  }

  setEventBtn(btnAll, markAll);
  setEventBtn(btnLovers, markLovers);
  setEventBtn(btnGirl, markGirl);
  setEventBtn(btnChef, markChef);
  setEventBtn(btnGuy, markGuy);
  setEventBtn(btnGrandad);
  setEventBtn(btnGrandmother);
...
var modals = function modals() {
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      ...
      if (!btnPressed && 10 + window.pageYOffset + document.documentElement.clientHeight >= scrollHeight) {
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    ...
    blocks.querySelectorAll('P:not(.sizes-hit)').forEach(function (p) {
      p.style.dispay = 'none';
    });
  }
...
var scrolling = function scrolling(upSelector) {
  ...
  }); // Scrolling with reqanimatedframe
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 2:
          ...
          throw new Error("Could not fetch ".concat(url, ", ststus ").concat(res.status));
